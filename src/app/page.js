...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                /* get banner data */
                const reqBannerData = await fetch(`${apiUrl}trending/all/day?api_key=${secretKey}&language=en-US`, options);
                ...
                const reqTrailer = await fetch(`${apiUrl}${resType}/${resId}/videos?api_key=${secretKey}&language=en-US`, options);
                ...
                /* get trending movies */
                const reqTrendingMovies = await fetch(`${apiUrl}trending/movie/day?api_key=${secretKey}&language=en-US`, options);
                const resTrendingMovies = await reqTrendingMovies.json();

                setTrendingMovieData(resTrendingMovies.results);

                /* get trending tv */
                const reqTrendingTv = await fetch(`${apiUrl}trending/tv/day?api_key=${secretKey}&language=en-US`, options);
                ...
                /* get search */
                if (query) {
                    const reqSearch = await fetch(`${apiUrl}search/multi?query=${query}&api_key=${secretKey}&include_adult=false&language=en-US&page=1`, options);
